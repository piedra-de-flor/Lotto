...
public class InputTest {
    ...
    @DisplayName("수동 로또 양 잘못된 입력 테스트 - 문자열 입력시")
    @ParameterizedTest
    @ValueSource(strings = {"가, 나, 다", "1, 2, 다", "ㅋㄴㅇ마ㅜㄴ"})
    void 수동_로또_양_잘못된_입력_테스트_문자열(String input) {
        Assertions.assertThrows(IllegalArgumentException.class, () -> {
            makeTestInputAmount(input, 10);
        });
    }

    @DisplayName("수동 로또 양 잘못된 입력 테스트 - 공백 입력시")
    void 수동_로또_양_잘못된_입력_테스트_공백() {
        Assertions.assertThrows(IllegalArgumentException.class, () -> {
            makeTestInputAmount(" ", 10);
        });
    }

    @DisplayName("수동 로또 양 잘못된 입력 테스트 - 자동 로또 양보다 클 시")
    void 수동_로또_양_잘못된_입력_테스트_자동보다_클_시() {
        Assertions.assertThrows(IllegalArgumentException.class, () -> {
            makeTestInputAmount("50", 1);
        });
    }

    @DisplayName("수동 로또 양 잘못된 입력 테스트 - 0이하 입력시")
    @ParameterizedTest
    @ValueSource(strings = {"-1", "-5", "-100"})
    void 수동_로또_양_잘못된_입력_테스트_범위_밖(String input) {
        Assertions.assertThrows(IllegalArgumentException.class, () -> {
            makeTestInputAmount(input, 10);
        });
    }

    private InputNumbers makeTestInput(String input) {
        return new InputNumbers(input);
    }

    private InputManualAmount makeTestInputAmount(String input, int autoAmount) {
        return new InputManualAmount(input, autoAmount);
    }
}
